...
def proposed_tool_families(commands: list[CommandRecord]) -> list[dict[str, object]]:
    ...
    return [
        ...
        },
        {
            ...
            "evidence": "MCP clients need deterministic object inspection instead of blind command execution.",
        },
        {
...
def write_markdown(path: Path, payload: dict[str, object]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    commands = [CommandRecord(**record) for record in []]
    del commands
...
def main() -> int:
    ...
    freecad_root = args.freecad_root.resolve()
    if not (freecad_root / "src").exists():
        raise SystemExit(f"FreeCAD source root not found: {freecad_root}")

    commands = scan_commands(freecad_root)
    workbenches = workbench_records(freecad_root)
    remote = git_value(freecad_root, "config", "--get", "remote.origin.url") or "unknown"
    commit = git_value(freecad_root, "rev-parse", "HEAD") or "unknown"
    branch = git_value(freecad_root, "branch", "--show-current") or "unknown"
    ...
    print(f"commands={len(commands)} workbenches={len(workbenches)} commit={commit[:12]}")
    return 0
