...
def modeling_strategy_preflight(params):
    source_type = normalized_strategy_value(params.get("source_type"))
    has_image = bool(params.get("has_image", False))
    strategy = normalized_strategy_value(params.get("modeling_strategy"))
    strategy_confirmed = bool(params.get("strategy_confirmed", False))
    visible_sketch_constraints = bool(params.get("visible_sketch_constraints", False))
    visible_dimensions = bool(params.get("visible_dimensions", False))
    visible_construction_geometry = bool(params.get("visible_construction_geometry", False))
    curves_visible = bool(params.get("curves_visible", False))
    visible_bspline_control_points = bool(params.get("visible_bspline_control_points", False))
    curve_intent = normalized_strategy_value(params.get("native_curve_intent"))
    curve_intent_confirmed = bool(params.get("curve_intent_confirmed", False))
    curve_intent_source = normalized_strategy_value(params.get("curve_intent_source"))
...
def enforce_strategy_lock_mode(params, lock_mode):
    preflight = modeling_strategy_preflight(params)
...
def sketch_modeling_strategy_report(params, sketch, *, validation=None, lock_mode=None, preflight=None):
    ...
    semantic_policy = bool(params.get("semantic_constraints", False)) or constraint_policy == "semantic"
    ...
    if strategy in STRICT_PARAMETRIC_STRATEGIES:
        ...
        if not bool(params.get("require_fully_constrained", False)):
...
def validate_sketch_profile(sketch, params):
    ...
    forbid_block_constraints = bool(params.get("forbid_block_constraints", False)) or profile_constraint_policy({}, params) == "semantic"
    include_construction = bool(params.get("include_construction", False))
...
def action_sketch_add_geometry(params):
    strategy_preflight = modeling_strategy_preflight(params)
    doc = get_doc(params)
    sketch = get_object(doc, params.get("sketch_name") or "")
    ...
    result = {
        ...
        "sketch": object_summary(sketch),
        "document": document_summary(doc),
        "modeling_strategy": sketch_modeling_strategy_report(params, sketch, preflight=strategy_preflight),
    }
...
def action_sketch_add_constraint(params):
    ...
    solver_evidence = sketch_constraint_solver_evidence(
        sketch,
        solve_code=solve_code,
        include_constraint_errors=bool(params.get("include_constraint_errors", False)),
    )
    return {
        "saved_path": saved,
        "added_indices": added,
...
def action_sketch_profile_validate(params):
    ...
    if any(key in params for key in ("source_type", "has_image", "modeling_strategy", "strategy_confirmed")):
        result["modeling_strategy"] = sketch_modeling_strategy_report(params, sketch, validation=validation)
    return result
...
def action_sketch_validate(params):
    ...
    if bool(params.get("include_geometry", True)):
