...
For image, screenshot, drawing, or visual-reference tasks, the server exposes a modeling strategy intake gate. Agents should ask whether the user expects visual similarity, an editable parametric sketch, a manufacturing PartDesign model, a Sketcher constraint rebuild, construction guides only, or a rough draft before mutating geometry. Sketcher mutation tools accept `source_type`, `modeling_strategy`, `strategy_confirmed`, visible Sketcher evidence flags, native curve intent fields, and line-fallback guard fields; image-like calls without a declared strategy stop before mutation. If a screenshot shows red dimensions/constraints or blue construction geometry, the safe default is constraint rebuild/editable sketch, not visual trace. Native B-spline/freeform profile creation is intentionally unsupported; if B-spline/freeform controls are visible, agents should ask for a supported arc/ellipse reinterpretation or create only construction guides, never silently approximate with many real lines or polylines.
...
- Deepen image-to-sketch analysis beyond the current modeling-strategy intake, especially supported arc/ellipse versus unsupported freeform decisions, line/polyline fallback prevention, and visible constraint/dimension extraction.
