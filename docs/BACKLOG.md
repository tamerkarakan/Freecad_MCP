...
- Add explicit FreeCAD 1.1 Sketcher external-reference aliases that match the GUI names: External Projection and External Intersection. The current typed operation is `freecad_sketch_edit_geometry` with `add_external`; future aliases should keep backward compatibility while making the 1.1 GUI mapping obvious to agents.
