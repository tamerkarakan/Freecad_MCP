...
- Expand GUI live bridge into full live FreeCAD access. Now that explicit `.FCStd` document open is covered, go beyond active document/view/selection/preselection/view-fit/viewport-snapshot into richer live drawing observation, command execution boundaries, transaction status, console forwarding, document dirty state, and safe GUI-side mutation policies.
