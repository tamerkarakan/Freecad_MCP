...
- Semantic rectangle constraint policy is implemented for process-per-call and worker profile creation: `constraint_policy="semantic"` adds named width/height/origin constraints, rejects `Block` shortcuts during validation, and the parametric profile recipe can auto-bind loop `width_expression`/`height_expression` before final DoF validation.
- Agent-facing modeling rules for complex sketches are implanted into MCP server instructions, the `freecad_design_task` prompt, process and worker Sketcher/PartDesign tool descriptions, `README.md`, `docs/SKETCHER_CAPABILITIES.md`, and resource `freecad://docs/agent-modeling-contract`: agents are told to use helper/profile recipes for known intents such as keyhole/slot/regular_polygon, avoid loose overlapping primitive profiles, reserve `trim` for edit/repair, and require semantic named dimensions/full-constraint validation for parametric sketches.
- The Sketcher method catalog now advertises common `Sketcher.Constraint(type, *values)` constructor type strings and field shapes through `constraint_methods`, including the `Group`/`Text` blocked-type policy, so agents do not have to infer the constraint surface from source.
- GUI bridge watchdog/heartbeat hardening is implemented: `freecad_gui_watchdog_status` reports session health, normal GUI calls mark sessions unhealthy after failures, bridge status includes heartbeat fields, and bridge API v4 honors per-call GUI-thread wait timeouts.
...
- Extract shared runtime helpers between the process-per-call `cad_action.py` script and persistent `worker.py` script so property validation, expression handling, summaries, and mesh safety cannot drift across the two FreeCAD runtimes.
- Add guarded expression preflight for `freecad_object_expression_set`/worker parity when implemented: lexer-level path/expression checks, clearer unit/Quantity diagnostics, and explicit opt-in for advanced FreeCAD expression syntax rather than blind `setExpression()` strings.
- Add bounded document summary modes to process-per-call runtime outputs, matching persistent-worker compact responses, so very large documents cannot force every tool call to serialize the full object tree unless the caller asks for it.
...
- Add a higher-level sketch-intent compiler/resource beyond guidance text: given an intent such as keyhole, socket array, mounting pattern, or rounded tab, return a preferred typed tool plan, helper loop payloads, required constraints, validation gates, and repair advice before the agent starts mutating a document.
- If needed later, derive a more exhaustive Sketcher constraint enum from the local FreeCAD source/runtime instead of relying only on the current common CAD-safe constructor catalog.
...
- Extend semantic constraint policy beyond the newly covered rectangle/polygon/circle/slot/keyhole helper loops: add fixture-backed named invariants for repeated socket arrays, external-reference anchoring, multi-profile spacing/count expression binding, and more human-edit-proof socket-table helpers so agents cannot satisfy `fully constrained` with static coordinates or `Block`.
