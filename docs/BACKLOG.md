...
- Extend GUI coverage according to `docs/GUI_1_1_1_RESEARCH.md`: after the first Sketcher/PartDesign state readers, deepen Sketcher edit enter/leave and PartDesign Body activation/task state before TechDraw GUI exporters/dimensions/projections, Assembly joint-from-selection/BOM/solve flows, and visual assist helpers such as Measure, clipping, screenshot, and reference-image calibration state. Keep CAM/FEM as guarded advanced expansions.
