...
- Extend GUI coverage according to `docs/GUI_1_1_1_RESEARCH.md`: after the first Sketcher edit enter/leave, PartDesign Body activation, and feature-task state tools, deepen task-specific preconditions before TechDraw GUI exporters/dimensions/projections, Assembly joint-from-selection/BOM/solve flows, and visual assist helpers such as Measure, clipping, screenshot, and reference-image calibration state. Keep CAM/FEM as guarded advanced expansions.
- Research and extend Sketcher and PartDesign coverage from both FreeCAD documentation and the local FreeCAD source checkout. Next PartDesign priorities are advanced Pipe options (sections, auxiliary spine, orientation/scaling), dress-up features, and pattern/mirror features on top of the implemented Body + Datum Plane + support-object sketch attachment foundation. Include GUI commands where safe through GUI/workbench bridge paths, and add typed wrappers only with source evidence, fixture-backed behavior, and smoke tests.
