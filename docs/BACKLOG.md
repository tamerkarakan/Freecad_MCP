...
- Extend semantic constraint policy beyond rectangle helpers: add fixture-backed named invariants for slots, keyholes, regular polygons/socket profiles, repeated socket arrays, external-reference anchoring, and multi-profile expression binding so agents cannot satisfy `fully constrained` with static coordinates or `Block`.
