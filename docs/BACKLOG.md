...
- Extend GUI coverage according to `docs/GUI_1_1_1_RESEARCH.md`: after the first Sketcher edit enter/leave and PartDesign Body activation tools, deepen task-panel state before TechDraw GUI exporters/dimensions/projections, Assembly joint-from-selection/BOM/solve flows, and visual assist helpers such as Measure, clipping, screenshot, and reference-image calibration state. Keep CAM/FEM as guarded advanced expansions.
