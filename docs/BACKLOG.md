...
- Research and extend Sketcher and PartDesign coverage from both FreeCAD documentation and the local FreeCAD source checkout. Next PartDesign priorities are Subtractive Loft and Pipe on top of the implemented Body + Datum Plane + support-object sketch attachment foundation. Include GUI commands where safe through GUI/workbench bridge paths, and add typed wrappers only with source evidence, fixture-backed behavior, and smoke tests.
