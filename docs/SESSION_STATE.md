...
- Sketcher typed tools now cover 11 tools: create, add geometry, add constraint, add profile, profile create, profile validate, edit geometry, edit constraints, transform, auto-constrain, and validate.
...
- `scripts/smoke_persistent_worker.py` now exercises Part, Sketcher connected closed chains, curve-preserving profile creation/rejection, mesh, and Assembly flows in one long-lived FreeCADCmd worker session.
