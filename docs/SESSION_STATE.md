...
- The two large embedded FreeCAD runtime scripts were extracted from inline triple-quoted literals into `src/freecad_mcp/runtime_scripts/{cad_action.py,worker.py}` and are loaded at import via `tooling.load_runtime_script`; `cad_tools.py`/`persistent_bridge.py` shrank from ~3325/~2780 to ~238/~351 lines. These script files are package data and must be committed/shipped alongside the modules that load them; the loader raises a clear error if either is missing.
...
7. Use `docs/ROADMAP_STATUS.md`, `docs/GUI_ATTACH_PLAN.md`, `docs/VISION_DEBUG_PIPELINE.md`, `docs/WORKBENCH_BRIDGE.md`, and `docs/TECHDRAW_CAM_FEM_PLAN.md` before extending GUI/vision/Workbench/TechDraw/CAM/FEM tools.
