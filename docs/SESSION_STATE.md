...
- `freecad_partdesign_body_create`/`freecad_partdesign_pad` and worker equivalents provide the first PartDesign typed slice: Body creation/reuse and Pad creation from an attached sketch, with optional solid-result enforcement and Body Tip reporting.
...
- Sellable bundle descriptors live in `freecad_mcp.product_bundles`; generated outputs are `docs/PRODUCT_BUNDLES.md` and `docs/product_bundles.json`, exposed as MCP resources. Current generated counts are Free 22, Pro 59, Studio 117, Team 120, Source add-on 5, and Unsafe add-on 1.
...
- Current MCP tool count is 121 when `FREECAD_MCP_MODULES=all`.
...
- `freecad_partdesign_pocket` cuts material from an existing Body solid (verifies the Body Tip is a solid first); CAD tool count 56.
...
- Current unit coverage includes `tests/unit/test_cad_tools.py` guards for CadToolService host-side dispatch/validation, the CAD domain-service split, `CAD_ACTION_SCRIPT` integrity, and runtime-script loading; `test_distribution_profiles.py` guards distribution profiles and pyproject packaging declarations; `test_workbench_artifact.py` guards the generated Workbench artifact manifest; worker session-cap and worker-script-integrity guards in `test_persistent_bridge.py`; and `safe_source_path` containment/escape guards in `test_static_tools.py`. Three previously tautological persistent-bridge selector-guard tests were de-tautologized with a real worker session plus message-matched assertions.
