...
- PartDesign workflows require sketches to live inside a Body and be attached to an origin plane. `freecad_sketch_create`/`freecad_sketch_profile_create` now accept `body_name` + `attachment_plane`, and `freecad_partdesign_pad` should be used when a Body feature is required instead of standalone `freecad_part_extrude`.
- Long mixed persistent-worker sessions can still expose FreeCAD global-state/recompute hangs around revolved PartDesign features. The worker smoke isolates Revolution/Groove in a clean worker session; prefer shorter worker sessions for heavy PartDesign feature chains until deeper lifecycle isolation is implemented.
