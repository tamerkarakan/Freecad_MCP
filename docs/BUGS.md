...
- Loop-based `freecad_sketch_profile_create` is the preferred mitigation for traced production profiles: it rejects non-colocated endpoints before constraining, validates Part face creation, and reports branch/micro-offset patterns instead of relying only on DoF/open-vertex metrics.
