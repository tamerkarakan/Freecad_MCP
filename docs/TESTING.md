...
- The persistent worker smoke restarts the worker after the Sketcher/PartDesign stress document before the later Part/mesh/Assembly document so unrelated Sketcher global state or recomputes do not mask the object, Part, mesh, or Assembly tool behavior under test.
