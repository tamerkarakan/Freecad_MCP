...
- The persistent worker smoke restarts the worker between PartDesign/early-Sketcher, PartDesign Revolution/Groove, PartDesign Additive Loft, PartDesign Subtractive Loft, Sketcher edit/transform, and later Part/mesh/Assembly slices so unrelated global state or recomputes do not mask the tool behavior under test.
