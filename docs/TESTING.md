...
- Real stdio MCP runtime mutation smoke through `scripts/smoke_mcp_runtime_stdio.py`, which creates a Part box and lists it through `tools/call` to guard against child FreeCADCmd inheriting the JSON-RPC stdin pipe.
...
- Static MCP smoke guard for GUI attach tool schemas (`freecad_gui_attach`, `freecad_gui_document_open`, `freecad_gui_visibility_ensure`, `freecad_gui_selection_get`, viewport snapshot, GUI object label set, Sketcher edit/state tools, PartDesign state/body activation tools, and feature-task state).
...
- Unit guard for GUI bridge attach/call/detach/error handling, GUI document-open/visibility-ensure/primitive-create/object-label/viewport-snapshot delegation, and Sketcher/PartDesign GUI state/edit/activation/task-state delegation against a fake local HTTP bridge (`test_gui_bridge.py`).
