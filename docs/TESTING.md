...
- Static MCP smoke guard for GUI attach tool schemas (`freecad_gui_attach`, `freecad_gui_document_open`, `freecad_gui_selection_get`, viewport snapshot, GUI object label set, Sketcher edit/state tools, PartDesign state/body activation tools, and feature-task state).
...
- Unit guard for GUI bridge attach/call/detach/error handling, GUI document-open/primitive-create/object-label/viewport-snapshot delegation, and Sketcher/PartDesign GUI state/edit/activation/task-state delegation against a fake local HTTP bridge (`test_gui_bridge.py`).
...
- Opt-in live FreeCAD GUI attach smoke (`FREECAD_MCP_GUI_SMOKE=1 scripts\verify.ps1` or `scripts/smoke_gui_attach.py`) launches FreeCAD GUI, starts the bridge, opens the saved `.FCStd` through `freecad_gui_document_open`, selects two Part faces, reads them via MCP GUI tools, sets a GUI object Label, enters/leaves a Sketcher edit session, reads feature-task state, activates a PartDesign Body, writes an active viewport snapshot, fits the view, closes the GUI process, and verifies a Fixed Assembly joint populated from those selection records.
