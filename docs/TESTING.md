...
- The persistent worker STL mesh import leg uses a tiny deterministic ASCII STL fixture and an explicit longer timeout because full verify runs it after package/build/typed-smoke work, where FreeCAD mesh import can be slower than the default worker request timeout on Windows.
