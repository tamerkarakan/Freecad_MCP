...
- Static MCP smoke guard for GUI attach tool schemas (`freecad_gui_attach`, `freecad_gui_selection_get`, Sketcher edit/state tools, PartDesign state/body activation tools, and feature-task state).
...
- Unit guard for GUI bridge attach/call/detach/error handling, GUI primitive-create delegation, and Sketcher/PartDesign GUI state/edit/activation/task-state delegation against a fake local HTTP bridge (`test_gui_bridge.py`).
...
- Opt-in live FreeCAD GUI attach smoke (`FREECAD_MCP_GUI_SMOKE=1 scripts\verify.ps1` or `scripts/smoke_gui_attach.py`) launches FreeCAD GUI, starts the bridge, selects two Part faces, reads them via MCP GUI tools, enters/leaves a Sketcher edit session, reads feature-task state, activates a PartDesign Body, fits the view, closes the GUI process, and verifies a Fixed Assembly joint populated from those selection records.
