...
- PartDesign Body/datum/support attachment plus Datum Plane, Pad, Pocket, Hole, Revolution, Groove, Additive/Subtractive Loft, and Additive/Subtractive Pipe creation for Sketcher profiles that need the same Body + plane/support workflow FreeCAD GUI prompts for. Pipe coverage includes guarded multisection section scaling and auxiliary-spine orientation paths.
