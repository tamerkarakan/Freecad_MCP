...
- PartDesign Body/datum/support attachment plus Datum Plane, Pad, Pocket, Hole, Revolution, Groove, Additive/Subtractive Loft, Additive/Subtractive Pipe, and Fillet/Chamfer/Thickness/Draft dress-up creation for Sketcher profiles and selected Body subelements that need the same Body + plane/support workflow FreeCAD GUI prompts for. Pipe coverage includes guarded multisection section scaling and auxiliary-spine orientation paths.
